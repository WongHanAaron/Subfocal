...
public:
	...
	/// <summary> Montage together several images within the specific size </summary>
	static cv::Mat Make(std::vector<cv::Mat> images, int dividerPixels = 0, int maxOutputWidth = -1, int maxOutputHeight = -1, cv::Scalar dividerColor = cv::Scalar(0, 0, 0));

	/// <summary> Gets the maximum image size and channel count. Returns (w, h, c) </summary>
	static std::tuple<int,int,int> _getMaxImageSize(std::vector<cv::Mat> images);
	...
	static cv::Mat _makeProc(std::vector<cv::Mat> images, std::tuple<int, int, int> maxImageSize, int cols, int rows, int dividerPixels = 0, int maxOutputWidth = -1, int maxOutputHeight = -1, cv::Scalar dividerColor = cv::Scalar(0, 0, 0));
};
